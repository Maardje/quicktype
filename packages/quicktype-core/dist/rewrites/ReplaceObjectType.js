...
export function replaceObjectType(graph, stringTypeMapping, _conflateNumbers, leaveFullObjects, debugPrintReconstitution) {
    function replace(setOfOneType, builder, forwardingRef) {
        ...
        function reconstituteProperties() {
            return mapMap(properties, cp => builder.makeClassProperty(builder.reconstituteTypeRef(cp.typeRef), cp.isOptional));
        }
        function makeClass() {
    ...
    const allObjectTypes = setFilter(graph.allTypesUnordered(), t => t.kind === "object");
